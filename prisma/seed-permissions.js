...
const permissions = [
  ...
  { code: 'consignment.dispatches.manage', name: 'Gestionar Despachos', group: 'Consignación' },
...
async function main() {
  ...
  await prisma.rolePermission.createMany({
    data: allPermissions.map((p) => ({ id_role: adminRole.id, id_permission: p.id })),
  });
  console.log(`Role "Administrador" assigned ${allPermissions.length} permissions.`);

  // Contador — accounting.* permissions
  ...
  await prisma.rolePermission.createMany({
    data: accountingPerms.map((p) => ({ id_role: contadorRole.id, id_permission: p.id })),
  });
  console.log(`Role "Contador" assigned ${accountingPerms.length} permissions.`);
  console.log('Done.');
}
