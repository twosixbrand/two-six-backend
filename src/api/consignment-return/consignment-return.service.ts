  ...
  async create(data: CreateReturnDto) {
    if (!data.items?.length) {
      throw new BadRequestException(
        'La devolución debe tener al menos 1 ítem.',
      );
    }
    ...
    if (data.return_type === 'POST_SALE') {
      ...
      if (!order)
        throw new NotFoundException(`Orden #${data.id_order} no encontrada.`);
      if (order.id_customer !== warehouse.id_customer) {
        throw new BadRequestException(
          'La orden no pertenece al cliente de esta bodega.',
        );
      }
      for (const it of data.items) {
    ...
    return this.prisma
      .$transaction(async (tx) => {
        ...
        for (const item of ret.items) {
          ...
          if (
            ret.return_type === 'PORTFOLIO' ||
            ret.return_type === 'WARRANTY'
          ) {
      ...
      .then(async (result) => {
        ...
        try {
          if (result.return_type === 'PORTFOLIO') {
            await this.journalAutoService.onConsignmentReturnPortfolio(
              result.id,
            );
          } else if (result.return_type === 'WARRANTY') {
            await this.journalAutoService.onConsignmentReturnWarranty(
              result.id,
            );
          } else if (result.return_type === 'POST_SALE') {
            await this.journalAutoService.onConsignmentReturnPostSale(
              result.id,
            );
          }
        } catch (err: any) {
