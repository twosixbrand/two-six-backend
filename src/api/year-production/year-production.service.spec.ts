...
describe('YearProductionService', () => {
  ...
  it('should create a year production', async () => {
    ...
    expect(mockPrisma.yearProduction.create).toHaveBeenCalledWith({
      data: dto,
    });
  });
  ...
  it('should find one year production', async () => {
    mockPrisma.yearProduction.findUnique.mockResolvedValue({
      id: '24',
      name: '2024',
    });
    const result = await service.findOne('24');
    expect(result?.id).toBe('24');
    expect(mockPrisma.yearProduction.findUnique).toHaveBeenCalledWith({
      where: { id: '24' },
    });
  });
  ...
  it('should update a year production', async () => {
    const dto = { name: '2025' };
    mockPrisma.yearProduction.update.mockResolvedValue({
      id: '24',
      name: '2025',
    });
  ...
  it('should remove a year production', async () => {
    ...
    expect(mockPrisma.yearProduction.delete).toHaveBeenCalledWith({
      where: { id: '1' },
    });
  });
});
