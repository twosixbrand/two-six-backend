...
export class DianUblService {
  ...
  private getDianExtensionsObj(docObj: any, invoiceObj: InvoiceDto, softwareSecurityCode: string, isNote: boolean = false) {
    const nit = this.config.get<string>('DIAN_COMPANY_NIT') || '';
    const dv = this.config.get<string>('DIAN_COMPANY_DV') || '';
    const name = (this.config.get<string>('DIAN_COMPANY_NAME') || '').replace(/"/g, '');
    ...
    const dianExt = extensions.ele('ext:UBLExtension')
      .ele('ext:ExtensionContent')
        .ele('sts:DianExtensions');

    // Notas Crédito y Débito no informan InvoiceControl en la extensión
    if (!isNote) {
      dianExt.ele('sts:InvoiceControl')
        .ele('sts:InvoiceAuthorization').txt(invoiceObj.resolutionNumber || '18760000001').up()
        .ele('sts:AuthorizationPeriod')
          .ele('cbc:StartDate').txt(invoiceObj.resolutionStartDate || '2023-01-01').up()
          .ele('cbc:EndDate').txt(invoiceObj.resolutionEndDate || '2030-12-31').up()
        .up()
        .ele('sts:AuthorizedInvoices')
          .ele('sts:Prefix').txt(invoiceObj.resolutionPrefix || 'SETP').up()
          .ele('sts:From').txt(String(invoiceObj.resolutionStartNumber || 1)).up()
          .ele('sts:To').txt(String(invoiceObj.resolutionEndNumber || 999999)).up()
        .up()
      .up();
    }

    dianExt.ele('sts:InvoiceSource')
    ...
    dianExt.ele('sts:SoftwareProvider')
      .ele('sts:ProviderID', { schemeAgencyID: '195', schemeAgencyName: 'CO, DIAN (Dirección de Impuestos y Aduanas Nacionales)', schemeID: dv, schemeName: '31' }).txt(nit).up()
      .ele('sts:SoftwareID', { schemeAgencyID: '195', schemeAgencyName: 'CO, DIAN (Dirección de Impuestos y Aduanas Nacionales)' }).txt(softwareId).up()
    .up();

    dianExt.ele('sts:SoftwareSecurityCode', { schemeAgencyID: '195', schemeAgencyName: 'CO, DIAN (Dirección de Impuestos y Aduanas Nacionales)' }).txt(softwareSecurityCode).up();

    dianExt.ele('sts:AuthorizationProvider')
      .ele('sts:AuthorizationProviderID', { schemeAgencyID: '195', schemeAgencyName: 'CO, DIAN (Dirección de Impuestos y Aduanas Nacionales)', schemeID: '4', schemeName: '31' }).txt('800197268').up()
    .up();

    dianExt.ele('sts:QRCode').txt(`https://catalogo-vpfe.dian.gov.co/document/searchqr?documentkey=CUFE_PLACEHOLDER`).up();

    dianExt.up().up().up();

    // 2. Extension para la firma digital (vacía para ser llenada por el SignerService)
    extensions.ele('ext:UBLExtension')
  ...
  private buildSupplierAndCustomer(doc: any, invoiceObj: InvoiceDto) {
    ...
    const customerParty = doc.ele('cac:AccountingCustomerParty')
      .ele('cbc:AdditionalAccountID').txt(customerAccountType).up()
      .ele('cac:Party')
        .ele('cac:PartyTaxScheme')
    ...
    if (customerAccountType === '2') { // Legal entity
      ...
      customerParty.ele('cac:Person')
        .ele('cbc:FirstName').txt(invoiceObj.customerName).up()
      .up();
    }
  ...
  generateCreditNoteXml(noteObj: NoteDto): string {
    ...
    const lines = noteObj.lines && noteObj.lines.length > 0
      ? noteObj.lines
    ...
    doc.ele('cbc:ProfileID').txt('DIAN 2.1: Nota Crédito de Factura Electrónica de Venta').up();
    doc.ele('cbc:ProfileExecutionID').txt(env === 'TEST' ? '2' : '1').up();
    doc.ele('cbc:ID').txt(noteObj.number).up();
    doc.ele('cbc:UUID', { schemeID: env === 'TEST' ? '2' : '1', schemeName: 'CUDE-SHA384' }).txt('CUFE_PLACEHOLDER').up();
    ...
    doc.ele('cbc:Note').txt(noteObj.reasonDesc || 'Nota Crédito').up();
    doc.ele('cbc:DocumentCurrencyCode').txt('COP').up();
    doc.ele('cbc:LineCountNumeric').txt(String(lines.length)).up();

    doc.ele('cac:DiscrepancyResponse')
      .ele('cbc:ReferenceID').txt(noteObj.originalInvoiceNumber).up()
      .ele('cbc:ResponseCode').txt(noteObj.reasonCode).up()
      .ele('cbc:Description').txt(noteObj.reasonDesc || 'Devolución').up()
    .up();

    doc.ele('cac:BillingReference')
      .ele('cac:InvoiceDocumentReference')
        .ele('cbc:ID').txt(noteObj.originalInvoiceNumber).up()
        .ele('cbc:UUID', { schemeName: 'CUFE-SHA384' }).txt(noteObj.originalInvoiceCufe).up()
        .ele('cbc:IssueDate').txt(noteObj.originalInvoiceDate).up()
      .up()
    .up();
  ...
  generateDebitNoteXml(noteObj: NoteDto): string {
    ...
    doc.ele('cbc:ProfileID').txt('DIAN 2.1: Nota Débito de Factura Electrónica de Venta').up();
    doc.ele('cbc:ProfileExecutionID').txt(env === 'TEST' ? '2' : '1').up();
    doc.ele('cbc:ID').txt(noteObj.number).up();
    doc.ele('cbc:UUID', { schemeID: env === 'TEST' ? '2' : '1', schemeName: 'CUDE-SHA384' }).txt('CUFE_PLACEHOLDER').up();
    doc.ele('cbc:IssueDate').txt(noteObj.date).up();
    doc.ele('cbc:IssueTime').txt(noteObj.time).up();
    doc.ele('cbc:Note').txt(noteObj.reasonDesc || 'Nota Débito').up();
    doc.ele('cbc:DocumentCurrencyCode').txt('COP').up();
    doc.ele('cbc:LineCountNumeric').txt(String(lines.length)).up();
    ...
    doc.ele('cac:DiscrepancyResponse')
      .ele('cbc:ReferenceID').txt(noteObj.originalInvoiceNumber).up()
      .ele('cbc:ResponseCode').txt(noteObj.reasonCode).up()
      .ele('cbc:Description').txt(noteObj.reasonDesc || 'Ajuste en precio').up()
    .up();
