...
export class DianCronService {
  ...
  async pollDianInvoices() {
    this.logger.debug(
      'Ejecutando polling de facturas DIAN (SENT -> AUTHORIZED)...',
    );

    const pendingInvoices = await this.prisma.dianEInvoicing.findMany({
      where: {
        OR: [{ status: 'SENT' }, { status: 'AUTHORIZED', email_sent: false }],
      },
    });
    ...
    for (const invoice of pendingInvoices) {
      if (invoice.status === 'AUTHORIZED' && invoice.email_sent === false) {
        try {
          this.logger.debug(
            `Retrying email for AUTHORIZED invoice ${invoice.document_number}`,
          );
          await this.emailService.sendDianInvoiceEmail(invoice.id);
        } catch (error) {
          this.logger.error(
            `Error retrying email for Factura ${invoice.id}:`,
            error,
          );
        }
        continue;
      ...
      try {
        const zipKeyMatch = invoice.dian_response.match(
          /<b:ZipKey>(.*?)<\/b:ZipKey>/,
        );
        ...
        if (!rawResponse || typeof rawResponse !== 'string') {
          this.logger.warn(
            `Respuesta vacía de GetStatusZip para factura ${invoice.document_number}`,
          );
          continue;
        }
        ...
        if (isValid && statusCode === '00') {
          ...
          newStatus = 'PROCESSED';
          this.logger.debug(
            `Factura ${invoice.document_number} procesada pero no autorizada aún (Estado: ${statusCode}).`,
          );
        } else if (statusCode === 'UNKNOWN') {
          // Si sigue pending
        }
        ...
        if (newStatus !== invoice.status) {
          ...
          await this.prisma.dianEInvoicing.update({
            ...
            data: {
              ...
              ...(newStatus === 'AUTHORIZED'
                ? { dian_authorized_at: new Date() }
                : {}),
            },
          });
          this.logger.log(
            `Factura ${invoice.document_number} actualizada a ${newStatus} por Cron Job.`,
          );
        ...
        this.logger.error(
          `Error en cron polling Factura ${invoice.id}:`,
          error,
        );
      }
    }
