...
import {
  ...
  InvoiceDto,
  NoteDto,
} from './dian-ubl/dian-ubl.service';
...
export class DianController {
  ...
  constructor(
    ...
    private readonly orchestratorService: DianOrchestratorService,
  ) {}
  ...
  @Get('status/:trackId')
  @ApiOperation({
    summary: 'Consultar estado de un documento en la DIAN por ZipKey/TrackId',
  })
  async getStatus(@Param('trackId') trackId: string) {
    try {
      ...
      const statusCode = rawResponse.match(
        /<b:StatusCode>(.*?)<\/b:StatusCode>/,
      )?.[1];
      const statusDescription = rawResponse.match(
        /<b:StatusDescription>(.*?)<\/b:StatusDescription>/,
      )?.[1];
      const statusMessage = rawResponse.match(
        /<b:StatusMessage>(.*?)<\/b:StatusMessage>/s,
      )?.[1];
      const isValid = rawResponse.match(/<b:IsValid>(.*?)<\/b:IsValid>/)?.[1];
      const errorMessages = rawResponse.match(
        /<b:ErrorMessage>(.*?)<\/b:ErrorMessage>/s,
      )?.[1];
      const xmlBase64Bytes = rawResponse.match(
  ...
  async getStatusXml(@Param('trackId') trackId: string, @Res() res: Response) {
    try {
      ...
      if (!xmlBase64Bytes) {
        throw new HttpException(
          'La respuesta de la DIAN no contiene XmlBase64Bytes',
  ...
  @ApiOperation({
    summary:
      'Genera y envía una factura electrónica a la DIAN (Consumo API Externa o Interno)',
  })
  @HttpCode(HttpStatus.OK)
  async createInvoice(@Body() body: any) {
    try {
      return await this.orchestratorService.generateAndSendInvoice(body);
    } catch (error) {
      this.logger.error(`Error generando factura: ${error.message}`);
      throw new HttpException(
        { error: error.message },
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
  ...
  @ApiOperation({
    summary: 'Generar y enviar Documento Soporte Electrónico para un Gasto',
  })
  async createSupportDocument(
    ...
    try {
      ...
      if (!resolution)
        throw new Error('No hay resolución configurada para Documento Soporte');
      ...
      const cuds = this.cufeService.generateCufe({
        NumFac: docDto.number,
        FecFac: docDto.date,
        HorFac: docDto.time,
        ValFac: docDto.subtotal!.toFixed(2),
        CodImp1: '01',
        ValImp1: docDto.taxTotal!.toFixed(2),
        CodImp2: '04',
        ValImp2: '0.00',
        CodImp3: '03',
        ValImp3: '0.00',
        ValTot: docDto.total!.toFixed(2),
        NitOfe: nit,
        NumAdq: docDto.customerDoc,
        ClTec: claveTecnica,
  ...
  @ApiOperation({
    summary: 'Reintenta generar y enviar una factura para una orden fallida',
  })
  @HttpCode(HttpStatus.OK)
  async retryInvoice(@Param('orderId') orderId: string, @Body() body: any) {
    try {
      ...
      if (!existingInvoice) {
        throw new Error(
          'No existe una factura previa para esta orden. Use generar factura normalmente.',
        );
      }
      ...
      const invoiceDto = {
        ...
        customerName:
          body.customerName || order?.customer?.name || 'Cliente Externo API',
        customerDoc:
          body.customerDoc ||
          order?.customer?.document_number ||
          '222222222222',
        customerDocType:
          body.customerDocType ||
          (order?.customer?.id_identification_type
            ? order.customer.id_identification_type.toString()
            : '13'),
        lines: orderLines,
        resolutionPrefix: resolution.prefix,
        resolutionNumber: resolution.resolutionNumber,
  ...
  async createCreditNote(@Param('id') id: string, @Body() body: any) {
    try {
      ...
      if (!resolution)
        throw new Error('No hay resolución configurada para Notas Crédito');
      ...
      const softwarePin =
        this.configService.get<string>('DIAN_SOFTWARE_PIN') || '';
      const nit = this.configService.get<string>('DIAN_COMPANY_NIT') || '';

      // 1. Calcular Totales para CUDE asegurando 2 decimales limpios
      const subtotalRaw = noteDto.lines!.reduce(
      ...
      const cude = this.cufeService.generateCude({
        NumNota: noteDto.number,
        FecNota: noteDto.date,
        HorNota: noteDto.time,
        ValNota: subtotalStr,
        CodImp1: '01',
        ValImp1: taxTotalStr,
        CodImp2: '04',
  ...
  async createDebitNote(@Param('id') id: string, @Body() body: any) {
    try {
      ...
      if (!resolution)
        throw new Error('No hay resolución configurada para Notas Débito');
      ...
      const subtotal = noteDto.lines!.reduce(
        (s, l) => s + l.quantity * l.unitPrice,
        0,
      );
  ...
  @ApiOperation({
    summary: 'Obtener Notas Crédito y Débito asociadas a una Factura',
  })
  ...
  @ApiOperation({
    summary: 'Consultar y actualizar el estado de una Nota con DIAN',
  })
  async syncNoteStatus(@Param('id') id: string) {
    try {
      ...
      if (!note || !note.status_message) {
        throw new Error(
          'Nota no encontrada o sin mensaje de respuesta DIAN original',
        );
      }

      // 1. Extraer ZipKey
      const zipKeyMatch = note.status_message.match(
        /<b:ZipKey>(.*?)<\/b:ZipKey>/,
      );
      if (!zipKeyMatch) {
        throw new Error(
          'No se encontró ZipKey en la respuesta original de la Nota',
        );
      }
      ...
      const isValid =
        rawResponse.match(/<b:IsValid>(.*?)<\/b:IsValid>/)?.[1] === 'true';
      const statusCode =
        rawResponse.match(/<b:StatusCode>(.*?)<\/b:StatusCode>/)?.[1] ||
        'UNKNOWN';
