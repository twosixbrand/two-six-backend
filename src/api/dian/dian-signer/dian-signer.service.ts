...
export class DianSignerService {
  ...
  signXml(xml: string): string {
    try {
      ...
      const certDigest = crypto.createHash('sha256').update(certBuffer).digest('base64');
      ...
      const issuerName = 'CN=OlimpiaIT ECD Sub, O=OlimpiaIT, OU=OlimpiaIT ECD, E=servicioalcliente@olimpiait.com, C=CO, S=Bogotá D.C., L=Bogotá D.C., SERIALNUMBER=900032774-4';
      ...
      let invoiceToDigest = xml.replace(/ Id="_0"/g, '');
      ...
      const invoiceDigest = crypto.createHash('sha256').update(invoiceC14n).digest('base64');
      ...
      const mockFinalXml = invoiceToDigest.replace('<ext:ExtensionContent/>', `<ext:ExtensionContent>${signatureBlockWrapper}</ext:ExtensionContent>`);
      const fullDoc = parser.parseFromString(mockFinalXml, 'text/xml');

      // Helper: recorrer ancestros y recolectar namespaces heredados
      function collectAncestorNamespaces(node: any): Array<{prefix: string, namespaceURI: string}> {
          ...
          if (parent.attributes) {
            for (let i = 0; i < parent.attributes.length; i++) {
              ...
              if (attr.nodeName && attr.nodeName.indexOf('xmlns') === 0) {
                const prefix = attr.nodeName === 'xmlns' ? '' : attr.nodeName.replace('xmlns:', '');
      ...
      const keyInfoNode = findNodeById(fullDoc.documentElement, `${sigId}-KeyInfo`);
      const signedPropsNode = findNodeById(fullDoc.documentElement, signedPropsId);
      ...
      const keyInfoContextC14n = c14n.process(keyInfoNode, { ancestorNamespaces: keyInfoAncestorNs });
      const signedPropsContextC14n = c14n.process(signedPropsNode, { ancestorNamespaces: signedPropsAncestorNs });

      const keyInfoDigest = crypto.createHash('sha256').update(keyInfoContextC14n).digest('base64');
      const signedPropsDigest = crypto.createHash('sha256').update(signedPropsContextC14n).digest('base64');

      // 7. Reemplazar Hashes en el XML string y volver a parsear para extraer SignedInfo
      let updatedMockFinalXml = mockFinalXml
        .replace('KEYINFO_PLACEHOLDER', keyInfoDigest)
        .replace('SIGNEDPROPS_PLACEHOLDER', signedPropsDigest);

      const updatedFullDoc = parser.parseFromString(updatedMockFinalXml, 'text/xml');
      ...
      const signedInfoContextC14n = c14n.process(signedInfoNode, { ancestorNamespaces: signedInfoAncestorNs });

      this.logger.debug('C14N SignedInfo (primeros 500 chars): ' + signedInfoContextC14n.substring(0, 500));
