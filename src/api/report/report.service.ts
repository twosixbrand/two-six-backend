  ...
  async getGeneralSalesReport(dto: GetGeneralSalesReportDto) {
    ...
    return orders.map((order) => ({
      ...
      customer: {
        ...
        identification: order.customer.identificationType
          ? `${order.customer.identificationType.name} ${order.customer.id}`
          : 'N/A', // Ojo: id es key, no el numero de documento. El schema no tiene 'document_number' en Customer? Reviemos schema.
      ...
      items: order.orderItems.map((item) => ({
        ...
        design_reference:
          item.product?.clothingSize?.clothingColor?.design?.reference || 'N/A',
      })),
    }));
  }
}
